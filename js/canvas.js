...
function renderOverlay() {
  ...
  if (isDraggingAdminShape && (tool === 'admin_rect' || tool === 'admin_circle' || tool === 'admin_line')) {
      ...
      if (tool === 'admin_rect') {
          ...
          let cxStart = Math.floor(adminShapeStart.x * camZoom + camZoom/2) + off.x;
          let cyStart = Math.floor(adminShapeStart.y * camZoom + camZoom/2) + off.y;
          let cxEnd = Math.floor(adminShapeEnd.x * camZoom + camZoom/2) + off.x;
          let cyEnd = Math.floor(adminShapeEnd.y * camZoom + camZoom/2) + off.y;
  ...
  if (tool === 'admin_move') {
      if (adminMoveState === 'select' && isDraggingAdminShape) {
          ...
          let rw = Math.floor(adminMoveRect.w * camZoom);
          let rh = Math.floor(adminMoveRect.h * camZoom);
  ...
  if ((tool==='admin_image' || adminImagePreviewMode) && adminImgObj) {
    let ir=adminImgRect;
    let sx=Math.floor(ir.x*camZoom)+off.x, sy=Math.floor(ir.y*camZoom)+off.y;
    let sw=Math.floor(ir.w*camZoom), sh=Math.floor(ir.h*camZoom);
  ...
  if (stencilActive && stencilImg) {
    ...
    if (purchasedItems.includes('stencil_auto_2') && hoveredPixel.x>=0 && !stencilEditMode) {
      renderStencilHints();
    }
  }
...
function renderStencilHints() {
  if (!stencilImageData || !stencilActive) return;
  const hx=hoveredPixel.x, hy=hoveredPixel.y;
  const ir=stencilRect;
  const sx=Math.floor((hx-ir.x)/ir.w*stencilImageData.width);
  const sy=Math.floor((hy-ir.y)/ir.h*stencilImageData.height);
  if (sx<0||sx>=stencilImageData.width||sy<0||sy>=stencilImageData.height) return;
  const idx=(sy*stencilImageData.width+sx)*4;
  const tr=stencilImageData.data[idx], tg=stencilImageData.data[idx+1], tb=stencilImageData.data[idx+2], a=stencilImageData.data[idx+3];
  ...
  dirs.forEach(([dx,dy])=>{
    ...
    const nsx=Math.floor((nx-ir.x)/ir.w*stencilImageData.width);
    const nsy=Math.floor((ny-ir.y)/ir.h*stencilImageData.height);
    if (nsx<0||nsx>=stencilImageData.width||nsy<0||nsy>=stencilImageData.height) return;
    const ni=(nsy*stencilImageData.width+nsx)*4;
    const nr=stencilImageData.data[ni],ng=stencilImageData.data[ni+1],nb=stencilImageData.data[ni+2],na=stencilImageData.data[ni+3];
    
    if (na > 50 && Math.abs(nr-tr)<30&&Math.abs(ng-tg)<30&&Math.abs(nb-tb)<30&&canvasData[ny*canvasW+nx]===0) {
      const rectSx = Math.floor(nx * camZoom) + off.x;
      const rectSy = Math.floor(ny * camZoom) + off.y;
      const rectEx = Math.floor((nx + 1) * camZoom) + off.x;
      const rectEy = Math.floor((ny + 1) * camZoom) + off.y;

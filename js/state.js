...
// Флаг "friends_get уже запрошен в этой сессии" — НЕ использовать
// cpFriends.length===0 как признак "надо запросить": у пользователя может
// быть 0 друзей, и тогда проверка на .length будет true после каждого
// ответа сервера, вызывая новый запрос -> новый friends_update -> новый
// renderProfileAchievementsTab() -> снова проверку .length===0 — бесконечный
// цикл перерисовки (см. баг ниже в ui.js).
let cpFriendsFetchedOnce = false;

...
function renderRanksList(containerId, xp) {
  ...
  const claimedCycles = new Set((claimedXpCycles || []).map(Number));
  const nextCycle = Array.from({ length: maxCycle }, (_, i) => i + 1).find(c => !claimedCycles.has(c)) || (maxCycle + 1);
  ...
  html += `<div class="road-node road-node-reward is-${canClaimRepeat ? 'current' : 'locked'}">
    <div class="road-card road-card-reward${canClaimRepeat ? ' claimable' : ''}" ${canClaimRepeat ? `data-xp-cycle="${nextCycle}" role="button" tabindex="0" title="Забрать 200 монет" aria-label="Забрать 200 монет за ${requiredXp.toLocaleString('ru-RU')} опыта"` : ''}>
...
let _ranksRoadIgnoreClick = false;
...
function setupRanksRoadDrag(containerId = 'profile-ranks-road-list') {
  ...
  window.addEventListener('mouseup', () => {
    if (!_ranksRoadDragState) return;
    _ranksRoadIgnoreClick = _ranksRoadDragState.moved;
    list.classList.remove('is-dragging');
    _ranksRoadDragState = null;
    if (_ranksRoadIgnoreClick) setTimeout(() => { _ranksRoadIgnoreClick = false; }, 0);
  });
  ...
  list.addEventListener('click', (e) => {
    if (_ranksRoadIgnoreClick || (_ranksRoadDragState && _ranksRoadDragState.moved)) { e.stopPropagation(); e.preventDefault(); return; }
    const card = e.target.closest('[data-xp-cycle]');
    if (!card || !list.contains(card)) return;
    const cycle = Number(card.dataset.xpCycle);
    if (Number.isInteger(cycle) && cycle > 0) claimXpCycleReward(cycle);
  }, true);
...
function renderProfileChangelogTab() {
  ...
  const versions = [
    { version:'v2026.08.05', title:'Надёжная синхронизация холста', items:[
      ...
      'Бомбочки и радужные штампы теперь сохраняются единым подтверждённым снимком до показа результата. Их клетки не должны исчезать после обновления страницы или перезапуска сервера.',
      'При обрыве Discord Activity сервер сразу фиксирует все уже принятые пиксели, а фоновые сохранения стали чаще. Быстрая рисовка с ускорителем лучше защищена от потери прогресса.',
      'Цвет пикселя и информация о его авторе теперь восстанавливаются только из одной завершённой версии холста. Больше не должно быть белых клеток с подписью человека, который ставил другой цвет.',
    ...
    { version:'v2026.08.04', title:'Большое обновление удобства и скорости', items:[
      ...
      'Трафарет стало удобнее настраивать: тяните за круглые маркеры по углам, чтобы изменить размер. Пропорции картинки можно оставить включёнными или снять одним нажатием.',
      'Под трафаретом появилась компактная панель с текущим размером, переключателем пропорций и кнопкой быстрого закрытия.',
      'Полноэкранный таймлапс получил новый строгий интерфейс плеера: таймлайн, скорость, навигация и экспорт собраны в одном меню по центру снизу.',
      'Админский раздел таймлапсов стал удобнее: библиотека записей и выбранный плеер теперь находятся рядом, а статус записи не мешает работе.',
      'Лидерборд стал удобнее: он показывает топ‑50 по опыту, обновляет цифры сам и всегда показывает ваше место внизу, даже если вы пока не в топе.',
      'Лидерборд больше не мигает при обновлении, не сбрасывает прокрутку и не перезапускает анимированные баннеры игроков.',
      'Повторяемая награда после звания «Бог Пикселей» исправлена: за каждые новые 1 000 опыта можно забрать 200 монет.'
    ]},
...
function activateItem(itemId) {
  ...
  renderItemQuickbar();
  // Выбор предмета не всегда сопровождается движением мыши. Без явной
  // перерисовки зона бомбочки появлялась только после следующего hover-события.
  renderOverlay();
}
...
function toggleStencilAspectLock() {
  if (stencilLocked) return;
  stencilAspectLocked = !stencilAspectLocked;
  const btn = document.getElementById('stencil-aspect-toggle');
  if (btn) {
    btn.classList.toggle('is-locked', stencilAspectLocked);
    btn.setAttribute('aria-pressed', String(stencilAspectLocked));
    btn.title = stencilAspectLocked ? 'Пропорции зафиксированы' : 'Пропорции свободны';
  }
  showToast(stencilAspectLocked ? 'Пропорции зафиксированы' : 'Пропорции можно менять свободно', 'info');
}
...
function handleStencilStart(clientX,clientY){
  if (!stencilActive||!stencilImg||!stencilEditMode||stencilLocked) return false;
  ...
  for (const [corner, point] of Object.entries(corners)) {
    if (Math.hypot(clientX - point[0], clientY - point[1]) <= hitRadius) {
      stencilHandle = `resize-${corner}`;
      stencilResizeStart = { rect: { ...ir }, ratio: ir.w / Math.max(1, ir.h) };
      return true;
    }
  }
...
function handleStencilMove(clientX,clientY){
  ...
  if (stencilHandle==='move'){
  ...
  } else if (stencilHandle.startsWith('resize-') && stencilResizeStart) {
    const corner = stencilHandle.slice(7);
    const start = stencilResizeStart.rect;
    const anchorX = corner.includes('w') ? start.x + start.w : start.x;
    const anchorY = corner.includes('n') ? start.y + start.h : start.y;
    let w = Math.max(1, Math.abs(p.x - anchorX));
    let h = Math.max(1, Math.abs(p.y - anchorY));
    if (stencilAspectLocked) {
      const ratio = stencilResizeStart.ratio;
      if (w / start.w >= h / start.h) h = w / ratio;
      else w = h * ratio;
    }
    ...
    stencilRect.w = w;
    stencilRect.h = h;
    stencilRect.x = corner.includes('w') ? Math.round(anchorX - w) : Math.round(anchorX);
    stencilRect.y = corner.includes('n') ? Math.round(anchorY - h) : Math.round(anchorY);
  }
  renderOverlay();
  return true;
}
...
let leaderboardRefreshQueued = null;
function requestLeaderboardRefresh(delay = 0) {
  if (!leaderboardOpen) return;
  if (delay <= 0) { sendJSON({action:'get_leaderboard'}); return; }
  clearTimeout(leaderboardRefreshQueued);
  leaderboardRefreshQueued = setTimeout(() => sendJSON({action:'get_leaderboard'}), delay);
}
function startLeaderboardRefresh() {
  clearInterval(leaderboardRefreshTimer);
  leaderboardRefreshTimer = setInterval(() => requestLeaderboardRefresh(), 4000);
}
function stopLeaderboardRefresh() {
  clearInterval(leaderboardRefreshTimer);
  clearTimeout(leaderboardRefreshQueued);
  leaderboardRefreshTimer = null;
  leaderboardRefreshQueued = null;
}
...
function switchLbTab(tab){
  ...
  document.getElementById('lb-players-view').style.display=tab==='players'?'':'none';
  document.getElementById('lb-clans-list').style.display=tab==='clans'?'':'none';
}

function leaderboardPlayerRowHtml(u, place, selfRow = false) {
  const b = profileBannerRowHTML(u.banner);
  const pos = Number(place) || 0;
  return `
    <div class="lb-row${b.cls}${selfRow ? ' lb-self-row' : ''}">
      ${b.html}
      <div class="lb-rank ${pos===1?'lb-rank-1':pos===2?'lb-rank-2':pos===3?'lb-rank-3':'lb-rank-n'}">${pos<4?['<svg class="icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M9 3h6l-1.3 5.4L12 11l-1.7-2.6L9 3z"/><circle cx="12" cy="15.5" r="5"/><path d="M9.7 14.3l1.6 1.6 2.8-2.8" stroke-width="1.7"/></svg>','<svg class="icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M9 3h6l-1.3 5.4L12 11l-1.7-2.6L9 3z"/><circle cx="12" cy="15.5" r="5"/><path d="M9.7 14.3l1.6 1.6 2.8-2.8" stroke-width="1.7"/></svg>','<svg class="icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M9 3h6l-1.3 5.4L12 11l-1.7-2.6L9 3z"/><circle cx="12" cy="15.5" r="5"/><path d="M9.7 14.3l1.6 1.6 2.8-2.8" stroke-width="1.7"/></svg>'][pos-1]:pos}</div>
      ...
      <div class="lb-capsule"><div class="lb-pixels">${(u.xp||0).toLocaleString()} XP</div></div>
    </div>`;
}

let leaderboardPlayersFingerprint = '';
function renderLeaderboardPlayers(data, self){
  const c=document.getElementById('lb-players-list');
  const selfBox = document.getElementById('lb-player-self');
  if (!c) return;
  // XP меняется часто, но не должен пересоздавать <img> баннеров: GIF/WebP
  // начинают анимацию заново при каждом innerHTML. Структуру списка (место,
  // игрок, аватар, баннер) отслеживаем отдельно от изменяемых чисел.
  const fingerprint = JSON.stringify({
    top: data.map(u => [u.username, u.place, u.rank, u.emoji, u.avatar, u.banner, u.online]),
    self: self ? [self.username, self.place, self.rank, self.emoji, self.avatar, self.banner, self.online] : null
  });
  if (fingerprint === leaderboardPlayersFingerprint) {
    const xp = c.querySelectorAll(':scope > .lb-row .lb-pixels');
    if (xp.length === data.length) data.forEach((u, i) => {
      xp[i].textContent = `${(u.xp || 0).toLocaleString()} XP`;
    });
    const ownXp = selfBox?.querySelector('.lb-pixels');
    if (ownXp && self) ownXp.textContent = `${(self.xp || 0).toLocaleString()} XP`;
    return;
  }
  leaderboardPlayersFingerprint = fingerprint;
  const scrollTop = c.scrollTop;
  c.innerHTML = data.length ? data.map((u, i) => leaderboardPlayerRowHtml(u, u.place || i + 1)).join('') : '<div style="color:var(--text3);text-align:center;padding:20px;">Пусто</div>';
  if (selfBox) selfBox.innerHTML = self
    ? `<div class="lb-self-label">Ваше место</div>${leaderboardPlayerRowHtml(self, self.place, true)}`
    : '';
...
let leaderboardClansFingerprint = '';
function renderLeaderboardClans(data){
  const c=document.getElementById('lb-clans-list');
  if (!c) return;
  const fingerprint = JSON.stringify(data.map((cl, i) => [i, cl.name, cl.tag, cl.icon, cl.tag_color, cl.banner_url, cl.banner_crop_x, cl.banner_crop_y, cl.banner_crop_w, cl.banner_crop_h]));
  if (fingerprint === leaderboardClansFingerprint) {
    const rows = c.querySelectorAll(':scope > .lb-row');
    if (rows.length === data.length) data.forEach((cl, i) => {
      const members = rows[i].querySelector('.lb-members');
      const pixels = rows[i].querySelector('.lb-pixels');
      if (members) members.lastChild.textContent = String(cl.members || 0);
      if (pixels) pixels.textContent = `${(cl.pixels || 0).toLocaleString()} px`;
    });
    return;
  }
  leaderboardClansFingerprint = fingerprint;
  ...
  c.innerHTML=data.map((cl,i)=>{
    ...
    const bannerCrop = { x: cl.banner_crop_x??0, y: cl.banner_crop_y??0, w: cl.banner_crop_w??1, h: cl.banner_crop_h??1 };
    return `
    <div class="lb-row${bannerUrl?' has-profile-banner':''}">
      ${bannerUrl ? clanBannerImgTag(bannerUrl, bannerCrop) : ''}
...
function getAdminUserSearch(){
  return (document.getElementById('admin-search')?.value || '').trim();
}
function loadAdminUsers(page){
  page = page || adminPage;
  sendJSON({action:'admin_cmd',cmd:'get_users',page,query:getAdminUserSearch()});
}
...
function renderAdminDMPlayers(users){
  const box = document.getElementById('admin-dm-players');
  if (!box) return;
  if (Array.isArray(users)) adminDMUsers = users;
  const query = (document.getElementById('admin-dm-search')?.value || '').trim().toLocaleLowerCase('ru-RU');
  const list = adminDMUsers.filter(u => String(u.username || '').toLocaleLowerCase('ru-RU').includes(query));
  if (!list.length) { box.innerHTML = `<div class="admin-dm-empty">${query ? 'Игроки не найдены' : 'Список игроков пока пуст'}</div>`; return; }
  box.innerHTML = list.map(u => `<button type="button" class="admin-dm-player ${u.username===adminDMSelectedUser?'is-selected':''}" data-onclick="adminDMSelect(${esc(JSON.stringify(String(u.username)))})">${cpAvatarHTML(u.username,'sm',{emoji:u.emoji,avatar:u.avatar,rank:u.rank,showDot:false})}<span>${esc(u.username)}</span>${u.online?'<i title="В сети"></i>':''}</button>`).join('');
}
function adminDMSelect(username){
  adminDMSelectedUser = String(username || '');
  const target = document.getElementById('dm-target');
  const selected = document.getElementById('admin-dm-selected');
  if (target) target.value = adminDMSelectedUser;
  if (selected) selected.innerHTML = adminDMSelectedUser ? `<span>Получатель</span><strong>${esc(adminDMSelectedUser)}</strong>` : 'Выберите получателя из списка';
  renderAdminDMPlayers();
}
function filterAdminDMPlayers(){
  renderAdminDMPlayers();
  clearTimeout(adminDMSearchTimer);
  adminDMSearchTimer = setTimeout(refreshAdminDMPlayers, 180);
}
function refreshAdminDMPlayers(){
  const query = (document.getElementById('admin-dm-search')?.value || '').trim();
  sendJSON({action:'admin_cmd', cmd:'get_users', page:1, query, recipient_picker:true});
}
...
function unlockCanvasDangerZone(){
  const input = document.getElementById('canvas-danger-password');
  const zone = document.getElementById('canvas-danger-zone');
  if (!input || !zone) return;
  if (input.value === '9786') {
    zone.classList.add('is-unlocked');
    input.value = '';
    showToast('Опасные действия разблокированы на эту сессию', 'success');
    return;
  }
  input.value = '';
  input.focus();
...
function renderAdminLockdownTab() {
  ...
  if (lockdownState.active) {
    ...
    if (title) title.textContent = 'Пиксель Батл закрыт';
    if (subtitle) subtitle.textContent = `Откроется через ${formatLockdownDuration(leftMs)}`;
  } else {
    box.classList.remove('is-locked');
    box.classList.add('is-open');
    if (title) title.textContent = 'Пиксель Батл открыт';
    if (subtitle) subtitle.textContent = 'Игроки могут размещать пиксели';
  }
}

function adminSetLockdownMinutes(minutes) {
  const input = document.getElementById('admin-lockdown-minutes');
  if (!input) return;
  input.value = Math.max(1, parseInt(minutes, 10) || 60);
  document.querySelectorAll('.lockdown-presets button').forEach(button => {
    button.classList.toggle('is-selected', Number(button.dataset.minutes) === Number(input.value));
  });
}
...
function hidePanel(id){
  ...
  document.getElementById('backdrop').classList.remove('show');
  if (id==='leaderboard-panel') stopLeaderboardRefresh();
...
function updateInspector(mx, my, px, py, fromCache) {
  // В режиме редактирования трафарета холст служит только рабочей областью:
  // не подсвечиваем клетки и не открываем карточку пикселя под курсором.
  if (stencilActive && stencilEditMode) { document.getElementById('inspector').style.display='none'; return; }
...
function renderAdminNewsList() {
  ...
  if (!box) return;
  const count = document.getElementById('admin-news-count');
  if (count) count.textContent = `${newsItems.length} ${newsItems.length === 1 ? 'публикация' : newsItems.length > 1 && newsItems.length < 5 ? 'публикации' : 'публикаций'}`;
  if (!newsItems.length) { box.innerHTML = `<div class="admin-news-empty"><div>✦</div><strong>Новостей пока нет</strong><span>Создайте первую публикацию для игроков</span></div>`; return; }

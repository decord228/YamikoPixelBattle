...
let tlSpeed = 1;            // множитель скорости (1 = реальное время)
let tlPlaying = false;
...
function tlRenderSessions() {
  const list = document.getElementById('tl-session-list');
  const count = document.getElementById('tl-session-count');
  if (count) count.textContent = String(tlSessions.length);
  ...
  list.innerHTML = tlSessions.map(s => {
    ...
    return `<div class="tl-session-item${selected}" data-onclick="tlSelectSession('${s.id}')">
      <div class="tl-sess-info">
        <div class="tl-sess-id">${started}</div>
...
function tlUpdateSelectedSessionUI() {
  const session = tlSessions.find(s => s.id === tlSelectedSessionId);
  const title = document.getElementById('tl-selected-session-title');
  const meta = document.getElementById('tl-selected-session-meta');
  if (!session || !title || !meta) return;
  const started = session.startedAt ? new Date(session.startedAt).toLocaleString() : session.id;
  const details = [];
  if (typeof session.totalEvents === 'number') details.push(`${session.totalEvents.toLocaleString()} событий`);
  if (session.w && session.h) details.push(`${session.w}×${session.h}`);
  title.textContent = started;
  meta.textContent = details.length ? details.join(' · ') : session.id;
}
...
function tlHandleSessionDeleted(id) {
  ...
  if (tlSelectedSessionId === id) {
    ...
    const empty = document.getElementById('tl-player-empty');
...
function tlFitMiniCanvas() {
  ...
  const maxW = Math.max(1, Math.min(640, (wrap?.clientWidth || 504) - 24));
  const maxH = Math.max(1, Math.min(480, (wrap?.clientHeight || 504) - 24));
  // Один коэффициент для обеих сторон: исходные пропорции холста не меняются
  // ни при узком окне, ни при изменении размера админской панели.
  const scale = Math.max(.05, Math.min(maxW / tlW, maxH / tlH));
  canvas.style.width = (tlW * scale) + 'px';
  canvas.style.height = (tlH * scale) + 'px';
}

window.addEventListener('resize', () => {
  if (!tlFullscreen) tlFitMiniCanvas();
});
...
function tlDrawMainCanvas() {
  ...
  // В новой системе отображения mainCanvas — это буфер, а видимый кадр
  // собирается overlay-рендерером. Просим его показать следующий кадр
  // таймлапса сразу после обновления буфера.
...
function tlDoCenterCamera() {
  ...
  // Снизу теперь расположен единый центрированный навигатор плеера,
  // поэтому оставляем ему больше безопасного пространства.
  const padTop = 64, padBottom = 174, padSide = 24;
...
function tlSyncPlayButtons() {
  const miniIcon = tlPlaying
    ? '<svg class="icon" viewBox="0 0 24 24"><path d="M8 6v12M16 6v12"/></svg>'
    : '<svg class="icon" viewBox="0 0 24 24"><path d="m9 6 8 6-8 6z" fill="currentColor" stroke="none"/></svg>';
  document.querySelectorAll('.tl-play-btn-sync').forEach(button => {
    if (button.id === 'tl-play-btn') button.innerHTML = miniIcon;
    else button.textContent = tlPlaying ? '⏸' : '▶';
    button.setAttribute('aria-label', tlPlaying ? 'Пауза' : 'Воспроизвести');
  });
}
